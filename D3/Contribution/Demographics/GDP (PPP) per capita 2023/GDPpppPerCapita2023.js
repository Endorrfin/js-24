...
// Helper function to parse gdp values (remove commas and convert to number)
function parseGdp(gdpStr) {
  if (typeof gdpStr === 'string') {
    return +gdpStr.replace(/,/g, '');
  }
  return +gdpStr;
}
...
async function loadData() {
  try {
    ...
    allData = csvData.map((d, i) => ({
      ...
      gdp: parseGdp(d['GDP (PPP) per capita 2023']),
      worldShare: parseFloat(d['world share'].replace('%', ''))
    })).filter(d => d.gdp > 0); // Filter out invalid data

    // Sort by gdp descending to ensure correct ranking
    allData.sort((a, b) => b.gdp - a.gdp);
...
// Create sample data if CSV loading fails
  function createSampleData() {
    const sampleData = [
      {rank: 1, region: 'Asia', flag: '🇸🇬', country: 'Singapore', GDPPPP: 141553, GDPNominal: 86616, worldShare: 620},
      {rank: 2, region: 'Europe', flag: '🇱🇺', country: 'Luxembourg', GDPPPP: 139106, GDPNominal: 128936, worldShare: 609},
      {rank: 3, region: 'Asia', flag: '🇶🇦', country: 'Qatar', GDPPPP: 128919, GDPNominal: 71500, worldShare: 565},
    ];
...
function updateVisualization() {
  ...
  g.append('text')
      ...
      .text('🌍 GDP (PPP) per capita 2023 $  vs World PPP GDP per capita $22,837 %')
      .style('fill', '#E4004B');
  ...
  // Add gdp labels on bars (updated format: number / percentage)
  g.selectAll('.gdp-label')
      .data(filteredData)
      .enter()
      .append('text')
      .attr('class', 'gdp-label')
      .attr('x', d => xScale(d.gdp) + 3)
      .attr('y', d => yScale(d.country) + yScale.bandwidth() / 2)
      .attr('dy', '0.5em')
      .style('font-size', '11px')
      .style('font-weight', 'bold')
      .style('fill', '#2c3e50')
      .text(d => `${formatNumber(d.gdp)} ⎪ ${d.worldShare.toFixed(0)}%`)
      .style('fill', '#004030')
}
...
function handleMouseMove(event, d) {
  const tooltipContent = `
                ...
                <div><strong>GDP (PPP) per capita:</strong> ${formatNumber(d.gdp)}</div>
                <div><strong>vs World PPP GDP per capita $22,837:</strong> ${d.worldShare.toFixed(0)}%</div>
            `;
