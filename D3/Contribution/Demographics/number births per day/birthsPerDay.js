...
async function loadData() {
  try {
    ...
    allData = data.map(d => ({
      ...
      flag: d.Flags,
      country: d.Country,
      births: parseFloat(d['Number of Births Per Day'].replace(/,/g, '')) // Remove commas and convert to number
    }));
...
function initializeChart() {
  ...
  chart.append('text')
      ...
      .style('text-anchor', 'middle')
      .text('Number of babies born per day');
}
...
function updateChart() {
  ...
  // Update scales
  xScale.domain([0, d3.max(batchData, d => d.births)]);
  yScale.domain(batchData.map(d => d.country));

  // Update axes
  const xAxisTicks = xScale.ticks(5);
  ...
  valueLabels.merge(valueLabelsEnter)
      ...
      .duration(750)
      .attr('y', d => yScale(d.country) + yScale.bandwidth() / 2)
      .attr('x', d => xScale(d.births) + 8)
      .text(d => d3.format(',')(d.births));
  ...
  tooltip.classed('show', true)
      .html(`
                    ...
                    <div style="margin-bottom: 4px;">
                        <strong>Births per day:</strong> ${d3.format(',')(d.births)}
                    </div>
                    <div style="margin-bottom: 4px;">
