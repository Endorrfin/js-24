...
const config = {
  ...
  regionColors: {
    ...
    'Europe': '#66D2CE',
    'Oceania': '#4E71FF',
    'Australia and New Zealand': '#5C7285'
  }
};
...
async function loadData() {
  try {
    ...
    allData = data.map(d => ({
      ...
      country: d.country,
      flag: d.flag,
      gpi: parseFloat(d['GPI'].replace(/,/g, ''))
    }));
...
function updateChart() {
  ...
  // Update scales
  xScale.domain([0, d3.max(batchData, d => d.gpi)]);
  ...
  bars.merge(barsEnter)
      ...
      .transition()
      .duration(2000) // CHANGED: Smoother animation
      .attr('y', d => yScale(d.country))
      .attr('height', yScale.bandwidth())
      .attr('width', d => xScale(d.gpi));
  ...
  valueLabels.merge(valueLabelsEnter)
      ...
      .duration(750)
      .attr('y', d => yScale(d.country) + yScale.bandwidth() / 2)
      .attr('x', d => xScale(d.gpi) + 10) // CHANGED: Better spacing
      .text(d => d3.format(',.1f')(d.gpi)); // CHANGED: Added decimal for precision
  ...
  tooltip.classed('show', true)
      .html(`
        ...
        <div style="margin-bottom: 2px;">
            <strong>GPI:</strong> ${d3.format(',.1f')(d.gpi)}
        </div>
        <div style="margin-bottom: 2px;">
