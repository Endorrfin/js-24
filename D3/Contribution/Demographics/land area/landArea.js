...
async function loadData() {
  try {
    ...
    allData = csvData.map((d, i) => ({
      ...
      area: parseArea(d['tatal area (Km²)']), // Note: keeping original column name with typo
...
function updateVisualization() {
  ...
  const xAxis = d3.axisBottom(xScale)
      .tickFormat(d => formatNumber(d)) // Removed km² from x-axis labels
      .ticks(8);
...
function handleMouseMove(event, d) {
  const tooltipContent = `
                ...
                <div><strong>Land area:</strong> ${formatNumber(d.area)} km²</div>
